...
async function authRoutes(fastify) {
	...
	fastify.post("/login", async (req, reply) => {
		...
		try {
			const user = await db.get("SELECT * FROM users WHERE name=?", [name]);
			if (!user)
				return reply.code(401).send({ field: "name", error: "INVALID USERNAME" });
			const valid = await argon2.verify(user.password, password);
			if (!valid)
				return reply.code(401).send({ field: "password", error: "INVALID PASSWORD" });
			...
			if (user.two_factor === 0) {
				...
				if (is2FAActive && twoFactorCode) {
					// user sent code TOTP in the header, verif it
			...
			const user_data = await db.get("SELECT id, name, species, planet, dimension, avatar, two_factor FROM users WHERE name=?", [name]);
